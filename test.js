...
.then(data => {
    ...
    let rank = ""
    if      (level < 10) rank = "Aspiring developer"
    else if (level < 20) rank = "Beginner developer"
    else if (level < 30) rank = "Apprentice developer"
    else if (level < 40) rank = "Assistant developer"
    else if (level < 50) rank = "Basic developer"
    else                 rank = "Junior developer"

    document.getElementById("current-rank").textContent = rank
    document.getElementById("level").textContent        = level
    document.getElementById("total-xp").textContent     = (totalXp / 1000).toFixed(1) + " KB"
    document.getElementById("avatar").src               = avatar
    document.getElementById("cohort").textContent       = cohort
    document.getElementById("fullname").textContent     = firstName + " " + lastName

    // only projects (type = "project"), no exercises
    const transactions = data.data.transaction.sort(
        (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
    )

    let cumulative = 0
    const slice = transactions.map(t => {
        cumulative += t.amount
        return {
            amount:     t.amount,
            cumulative: cumulative,
            project:    t.object.name,
        }
    })
